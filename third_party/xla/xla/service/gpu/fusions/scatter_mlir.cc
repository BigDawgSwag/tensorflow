               ...
               return EmitXlaLoopOp(
                   ...
                   [&](ImplicitLocOpBuilder& update_loop_b,
                     ...
                     return WriteAccumulatedElementToOutput(
                         ...
                         slice_indices, offsets, output_tensor);
                   });
             })
      .front();
}
...
ScatterWithDistributedUpdates::ScatterWithDistributedUpdates(
  ...
  VLOG(5) << "Settings for ScatterWithDistributedUpdates: \n"
...
ScatterWithDistributedIndices::ScatterWithDistributedIndices(
  ...
  VLOG(5) << "Settings for ScatterWithDistributedIndices: \n"
          << "vector_size_: " << vector_size_ << "\n"
          << "num_warps_: " << num_warps_ << "\n"
          << "num_blocks_: " << num_blocks_
...
Value ScatterWithDistributedIndices::InitializeAccumulator(
  ...
  auto splat = elem_type.isInteger()
                   ? mlir::DenseElementsAttr::get(accumulator_type, int{0})
                   : mlir::DenseElementsAttr::get(accumulator_type, float{0});
  return b.create<arith::ConstantOp>(accumulator_type, splat);
}
...
absl::Status ScatterWithDistributedIndices::EmitEntryFunctionImpl(
  ...
  auto loop_over_indices_fn = [&](ImplicitLocOpBuilder& nested_b,
    ...
    Value is_inbounds = iter_args_unpack[1].front();
    Value accumulator = iter_args_unpack[2].front();
    Value output_tensor = iter_args_unpack[3].front();
    Value loop_over_indices_iv = ivs.front();
    ...
    Value is_not_first_iteration =
        b.create<arith::CmpIOp>(arith::CmpIPredicate::ne, loop_over_indices_iv,
                                b.create<arith::ConstantIndexOp>(0));
    Value write_to_output_required = b.create<arith::AndIOp>(
        is_not_first_iteration,
        b.create<arith::AndIOp>(offsets_changed, is_inbounds));
    output_tensor = helper.WriteAccumulatorToOutput(
        b, write_to_output_required, thread_and_block_ids, loop_over_indices_iv,
        slice_indexing, offsets_changed, offsets, accumulator, output_tensor);

    // Update `is_inbounds` if the offsets changed.
    is_inbounds =
        UpdateIsInbounds(nested_b, is_inbounds, offsets_changed, new_offsets,
    ...
    auto is_last_iteration = nested_b.create<arith::CmpIOp>(
        arith::CmpIPredicate::eq, loop_over_indices_iv,
        b.create<arith::ConstantIndexOp>(num_indices_per_warp_ - 1));

    SmallVector<Value> acc_and_output = {accumulator, output_tensor};
          ...
          EmitUpdateIf(update_loop_b, is_last_iteration, output_arg,
                       [&](ImplicitLocOpBuilder& nested_b) {
                         return helper.WriteAccumulatedElementToOutput(
                             ...
                             slice_indices, new_offsets, output_tensor);
                       })
              .front();
      return {updated_accumulator, updated_output};
    };
    auto updated_accumulator_and_output = EmitUpdateIf(
        nested_b, is_inbounds, acc_and_output, [&](ImplicitLocOpBuilder& if_b) {
          return EmitXlaLoopOp(
              if_b, Pack({thread_and_block_ids, loop_over_indices_iv}),
              acc_and_output, slice_indexing, loop_over_slices_fn);
        });
    SmallVector<Value> updated_if_loop_results = Pack(
        {new_trimmed_offsets, is_inbounds, updated_accumulator_and_output});
  ...
  auto op = b.create<ReturnOp>(loop_over_indices_results.back());
...
std::unique_ptr<MlirScatterFusion> CreateMlirScatterFusion(
  ...
  // For sorted scatter, we try to the ration of the number of the given updates
  // to the number of the possible valid indices. If we do not have multiple
  // updates per warp, there is no reason to use this algorithm.
  if (description.scatter->indices_are_sorted() &&
      description.elem_type == F32 && num_slices > 2 * max_active_warps) {
